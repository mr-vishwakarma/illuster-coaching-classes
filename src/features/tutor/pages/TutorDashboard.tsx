...
const TutorDashboard = () => {
  ...
  // Live sessions from shared hook (real-time)
  const { liveSessions: activeSessions } = useLiveSessions(user?.id);
  ...
  useEffect(() => {
    ...
    const fetchTutorData = async () => {
      setIsLoading(true);
      try {
        const [{ data: ended }, { data: doubts }] = await Promise.all([
        ...
        setStats(prev => ({
          ...prev,
          totalSessions: activeSessions.length + (ended?.length || 0),
